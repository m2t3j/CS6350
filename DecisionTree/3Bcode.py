...
#3b. Same code as 3a but one more function

def impute_unknown(data, categorical_attributes):
    for attr in categorical_attributes:
        # most_common_value = data[attr].mode()[0]
        # if data[attr] == "unknown":
        #     data[attr] = most_common_value
        most_common_value = data[attr].value_counts().idxmax()
        data[attr] = data[attr].replace('unknown', most_common_value)
    return data
...
categorical_attributes = ['job', 'marital', 'education', 'default', 'housing', 'loan', 
                          'contact', 'month', 'poutcome']

train_data = impute_unknown(train_data,categorical_attributes= categorical_attributes)
test_data = impute_unknown(test_data,categorical_attributes= categorical_attributes)

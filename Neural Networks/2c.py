...
def initialize_weights(input_size, hidden_sizes, output_size, zero_init=False):
    weights = []
    biases = []
...
def stochastic_gradient_descent(X_train, y_train, X_test, y_test, input_size, output_size, hidden_widths, gamma0, d, epochs, zero_init=False):
    training_errors = {}
    test_errors = {}

    for width in hidden_widths:
        hidden_sizes = [width, width]
        weights, biases = initialize_weights(input_size, hidden_sizes, output_size, zero_init)
...
# Run with zero-initialized weights
training_errors_zero, test_errors_zero = stochastic_gradient_descent(
    X_train, y_train, X_test, y_test, input_size, output_size, hidden_widths, gamma0, d, epochs, zero_init=True
)

print("Training Errors with Zero Initialization:", training_errors_zero)
print("Test Errors with Zero Initialization:", test_errors_zero)
print("\n")
print("2c answer: Zero initialization results in significantly higher training and test errors compared to random initialization. This is probably due to the fact that each neuron starts out the same.")
print('\n')
